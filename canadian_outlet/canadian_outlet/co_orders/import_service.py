...
def import_order(order):
	...
	import_key = f"{channel}::{channel_order_id}"
	payload_hash = _payload_hash(order)

	# INV-11A: enforcement is the unique (co_sales_channel, co_channel_order_id)
	# pair on Sales Order (docs/DATA-MODEL.md §4); the log is the evidence trail.
	...
	if existing:
		_write_log(channel, channel_order_id, import_key, "Duplicate", payload_hash,
	...
	try:
		...
		_write_log(channel, channel_order_id, import_key, "Exception", payload_hash,
			integration_exception=exception_name)
		return ImportResult("Exception", integration_exception=exception_name)

	_write_log(channel, channel_order_id, import_key, "Created", payload_hash,
